...
{
	pj_str_t dial_source_number = pj_str(DIAL_TONE_SOURCE_NUMBER);
...
static void on_call_state(pjsua_call_id call_id, pjsip_event *e)
{
	...
	if (call_info.state == PJSIP_INV_STATE_DISCONNECTED)
	{
		...
		else
		{
			...
			if ((current_call_data->answer_delay_timer.id != PJSUA_INVALID_ID) && (call_info.last_status != PJSIP_SC_NOT_FOUND))
			{
				pjsip_endpoint *endpt = pjsua_get_pjsip_endpt();
				pjsip_endpt_cancel_timer(endpt, &current_call_data->answer_delay_timer);
				current_call_data->answer_delay_timer.id = PJSUA_INVALID_ID;
			}
			if ((current_call_data->call_timeout_timer.id != PJSUA_INVALID_ID) && (call_info.last_status != PJSIP_SC_NOT_FOUND))
			{
				pjsip_endpoint *endpt = pjsua_get_pjsip_endpt();
